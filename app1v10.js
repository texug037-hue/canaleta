...
const APP_VERSION = "v1.0.2 · app1v10/app2v10/app3v10";
...
async function salvarMaintenanceFirebase(orders) {
  try {
    localStorage.setItem("canaleta_maintenance", JSON.stringify(orders));
  } catch (e) {}
  try {
    if (window._fbReady && window._fbDb) {
      await window._fbDb.collection("canaleta").doc("maintenance").set({
        data: JSON.stringify(orders),
        updatedAt: Date.now()
      });
    }
  } catch (e) {
    console.warn("Firebase maintenance save error:", e);
  }
}
async function carregarMaintenanceFirebase() {
  try {
    if (window._fbReady && window._fbDb) {
      const snap = await window._fbDb.collection("canaleta").doc("maintenance").get();
      if (snap.exists) {
        const parsed = JSON.parse(snap.data().data);
        localStorage.setItem("canaleta_maintenance", JSON.stringify(parsed));
        return parsed;
      }
    }
  } catch (e) {
    console.warn("Firebase maintenance load error:", e);
  }
  try {
...
async function salvarOperatorsFirebase(ops) {
  try {
    localStorage.setItem("canaleta_operators", JSON.stringify(ops));
  } catch (e) {}
  try {
    if (window._fbReady && window._fbDb) {
      await window._fbDb.collection("canaleta").doc("operators").set({
        data: JSON.stringify(ops),
        updatedAt: Date.now()
      });
    }
  } catch (e) {
    console.warn("Firebase operators save error:", e);
  }
}
async function carregarOperatorsFirebase() {
  try {
    if (window._fbReady && window._fbDb) {
      const snap = await window._fbDb.collection("canaleta").doc("operators").get();
      if (snap.exists) {
        const parsed = JSON.parse(snap.data().data);
        localStorage.setItem("canaleta_operators", JSON.stringify(parsed));
        return parsed;
      }
    }
  } catch (e) {
    console.warn("Firebase operators load error:", e);
  }
  try {
    const r = localStorage.getItem("canaleta_operators");
    if (r) return JSON.parse(r);
  } catch (e) {}
  return [];
}
async function salvarProductsFirebase(prods) {
  try {
    localStorage.setItem("canaleta_products", JSON.stringify(prods));
  } catch (e) {}
  try {
    if (window._fbReady && window._fbDb) {
      await window._fbDb.collection("canaleta").doc("products").set({
        data: JSON.stringify(prods),
        updatedAt: Date.now()
      });
    }
  } catch (e) {
    console.warn("Firebase products save error:", e);
  }
}
async function carregarProductsFirebase() {
  try {
    if (window._fbReady && window._fbDb) {
      const snap = await window._fbDb.collection("canaleta").doc("products").get();
      if (snap.exists) {
        const parsed = JSON.parse(snap.data().data);
        localStorage.setItem("canaleta_products", JSON.stringify(parsed));
        return parsed;
      }
    }
  } catch (e) {
    console.warn("Firebase products load error:", e);
  }
  try {
    const r = localStorage.getItem("canaleta_products");
    if (r) return JSON.parse(r);
  } catch (e) {}
  return [];
}
async function salvarLoginLogFirebase(log) {
  try {
    localStorage.setItem("canaleta_loginlog", JSON.stringify(log));
  } catch (e) {}
  try {
    if (window._fbReady && window._fbDb) {
      await window._fbDb.collection("canaleta").doc("loginlog").set({
        data: JSON.stringify(log),
        updatedAt: Date.now()
      });
    }
  } catch (e) {
    console.warn("Firebase loginlog save error:", e);
  }
}
async function carregarLoginLogFirebase() {
  try {
    if (window._fbReady && window._fbDb) {
      const snap = await window._fbDb.collection("canaleta").doc("loginlog").get();
      if (snap.exists) {
        const parsed = JSON.parse(snap.data().data);
        localStorage.setItem("canaleta_loginlog", JSON.stringify(parsed));
        return parsed;
      }
    }
  } catch (e) {
    console.warn("Firebase loginlog load error:", e);
  }
  try {
...
async function salvarRegistroFirebase(machineId, machineName, rec) {
  try {
    if (window._fbReady && window._fbDb) {
      const id = rec.recordId || machineId + "_" + Date.now();
...
function sameHistoryRecord(a, b) {
  ...
  return (a.date || "") === (b.date || "") &&
    (a.turno || "") === (b.turno || "") &&
    (a.maquina || a._name || "") === (b.maquina || b._name || "") &&
    (a.produto || "") === (b.produto || "") &&
    (a.orderId || "") === (b.orderId || "");
}

async function excluirRegistrosFirebase(rec) {
  try {
    ...
    let count = 0;
    snap.forEach((doc) => {
      if (sameHistoryRecord(doc.data() || {}, rec)) {
    ...
    console.warn("Firebase histórico delete error:", e);
  }
}
...
const USERS = [
  ...
  { user: "demo", pass: "4209", name: "Demonstra\xE7\xE3o", role: "admin", isDemo: true }
];
function canDo(session, action) {
  if (session == null ? void 0 : session.isDemo) return action !== "delete";
...
function InstallIcon() {
  return /* @__PURE__ */ React.createElement("svg", { width: 22, height: 22, viewBox: "0 0 24 24", fill: "none", style: { flexShrink: 0 } }, /* @__PURE__ */ React.createElement("rect", { x: 7, y: 2, width: 10, height: 18, rx: 2, stroke: "#3ad080", strokeWidth: 1.6 }), /* @__PURE__ */ React.createElement("line", { x1: 10, y1: 17, x2: 14, y2: 17, stroke: "#3ad080", strokeWidth: 1.6, strokeLinecap: "round" }), /* @__PURE__ */ React.createElement("path", { d: "M9 10l3 3 3-3", stroke: "#3ad080", strokeWidth: 1.6, strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ React.createElement("line", { x1: 12, y1: 6, x2: 12, y2: 13, stroke: "#3ad080", strokeWidth: 1.6, strokeLinecap: "round" }));
}
function CalculatorIcon({ size = 22, color = "#5aa8e0" }) {
  return /* @__PURE__ */ React.createElement("svg", { width: size, height: size, viewBox: "0 0 24 24", fill: "none", style: { flexShrink: 0 } }, /* @__PURE__ */ React.createElement("rect", { x: 4.5, y: 2.5, width: 15, height: 19, rx: 2.2, stroke: color, strokeWidth: 1.6 }), /* @__PURE__ */ React.createElement("rect", { x: 7, y: 5, width: 10, height: 4, rx: 0.8, fill: color, fillOpacity: 0.18, stroke: color, strokeWidth: 1.3 }), [12.3, 16.1, 19.9].map((cy) => /* @__PURE__ */ React.createElement(React.Fragment, { key: cy }, [7.7, 12, 16.3].map((cx) => /* @__PURE__ */ React.createElement("circle", { key: cx, cx, cy, r: 1, fill: color })))));
}
function ProductionCalcModal({ onClose }) {
  const [horas, setHoras] = useState("");
  const [minutos, setMinutos] = useState("");
  const [ciclo, setCiclo] = useState("");
  const [pecasCiclo, setPecasCiclo] = useState("");
  const [pecasCaixa, setPecasCaixa] = useState("");
  const [caixasFardo, setCaixasFardo] = useState("");

  const h = parseInt(horas, 10) || 0;
  const m = parseInt(minutos, 10) || 0;
  const c = parseFloat(ciclo) || 0;
  const pc = parseInt(pecasCiclo, 10) || 1;
  const pcx = parseInt(pecasCaixa, 10) || 1;
  const cf = parseInt(caixasFardo, 10) || 1;

  const segundosTotais = h * 3600 + m * 60;
  const ciclos = c > 0 ? Math.floor(segundosTotais / c) : 0;
  const pecas = ciclos * pc;
  const caixasExatas = pecas / pcx;
  const caixas = Math.ceil(caixasExatas);
  const fardosExatos = caixas / cf;
  const fardos = Math.ceil(fardosExatos);

  function numInput(label, value, setValue, opts) {
    return /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#5a7aa0", letterSpacing: ".1em", marginBottom: 6 } }, label), /* @__PURE__ */ React.createElement("div", { style: { background: "#131a28", border: "1px solid #232f42", borderRadius: 10, padding: "10px 12px" } }, /* @__PURE__ */ React.createElement("input", { type: "number", inputMode: "decimal", value, onChange: (e) => setValue(e.target.value), placeholder: "0", min: (opts == null ? void 0 : opts.min) != null ? opts.min : 0, style: { background: "transparent", border: "none", color: "#e8eeff", fontSize: 16, width: "100%", fontFamily: "monospace" } })));
  }
  function sectionLabel(txt) {
    return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#3a5a7a", letterSpacing: ".12em", margin: "14px 0 8px", textTransform: "uppercase" } }, txt);
  }

  return /* @__PURE__ */ React.createElement("div", { style: { position: "fixed", inset: 0, background: "#000c", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }, onClick: onClose }, /* @__PURE__ */ React.createElement("div", { style: { background: "#0d1420", border: "1px solid #22314a", borderRadius: 18, width: "100%", maxWidth: 400, maxHeight: "92vh", overflowY: "auto", padding: "22px 20px 24px", fontFamily: "monospace" }, onClick: (e) => e.stopPropagation() }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 800, color: "#e8eeff", display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement(CalculatorIcon, { size: 18 }), "Calculadora de Produ\xE7\xE3o"), /* @__PURE__ */ React.createElement("button", { onClick: onClose, style: { background: "transparent", border: "none", color: "#5a6a8a", fontSize: 20, cursor: "pointer" } }, "\u2715")), sectionLabel("\u23F1 Tempo de produ\xE7\xE3o"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, marginBottom: 16 } }, numInput("HORAS", horas, setHoras), numInput("MINUTOS", minutos, setMinutos, { min: 0 })), sectionLabel("\u2699 Ciclo da m\xE1quina"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, marginBottom: 16 } }, numInput("CICLO (SEGUNDOS)", ciclo, setCiclo, { min: 0.1 }), numInput("PE\xC7AS POR CICLO", pecasCiclo, setPecasCiclo, { min: 1 })), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4, background: "linear-gradient(135deg,#122040,#0d1830)", border: "1px solid #2a5ab066", borderRadius: 14, padding: 16, textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#5a7aa0", letterSpacing: ".1em", marginBottom: 6 } }, "PE\xC7AS PRODUZIDAS"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 36, fontWeight: 900, color: "#5ad0ff", lineHeight: 1 } }, pecas.toLocaleString("pt-BR")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, color: "#4a6a8a", marginTop: 4 } }, "p\xE7s")), /* @__PURE__ */ React.createElement("div", { style: { borderTop: "1px dashed #1e2a3a", margin: "16px 0 2px" } }), sectionLabel("\u{1F4E6} Embalagem"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, marginBottom: 6 } }, numInput("PE\xC7AS POR CAIXA", pecasCaixa, setPecasCaixa, { min: 1 }), numInput("CAIXAS POR FARDO", caixasFardo, setCaixasFardo, { min: 1 })), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, marginTop: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1, background: "#0f1a2c", border: "1px solid #22314a", borderRadius: 12, padding: "12px 10px", textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#4a6a8a", letterSpacing: ".1em", marginBottom: 4 } }, "CAIXAS NECESS\xC1RIAS"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, fontWeight: 900, color: "#7ad0a0" } }, caixas.toLocaleString("pt-BR")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#3a5a7a", marginTop: 2 } }, caixasExatas.toFixed(2).replace(".", ","), " exatas")), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, background: "#0f1a2c", border: "1px solid #22314a", borderRadius: 12, padding: "12px 10px", textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#4a6a8a", letterSpacing: ".1em", marginBottom: 4 } }, "FARDOS A ABRIR"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, fontWeight: 900, color: "#e0a860" } }, fardos.toLocaleString("pt-BR")), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#3a5a7a", marginTop: 2 } }, fardosExatos.toFixed(2).replace(".", ","), " exatos"))), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 12, fontSize: 11, color: "#4a5a7a", textAlign: "center", lineHeight: 1.7 } }, h, "h", String(m).padStart(2, "0"), "min \u2192 ", ciclos.toLocaleString("pt-BR"), " ciclos \xD7 ", pc, " p\xE7/ciclo = ", pecas.toLocaleString("pt-BR"), " p\xE7s \u2192 \xF7", pcx, " p\xE7s/caixa \u2192 \xF7", cf, " caixas/fardo")));
}
function LoginScreen({ onLogin, users: propUsers, onExport, onImport }) {
  ...
  ), /* @__PURE__ */ React.createElement("button", { onClick: doLogin, disabled: loading, style: { width: "100%", background: loading ? "#1a2636" : "linear-gradient(135deg,#1a3a6a,#2a5aaa)", border: "1px solid #4a8aff44", color: loading ? "#3a5a7a" : "#e0f0ff", borderRadius: 10, padding: "13px 0", cursor: loading ? "default" : "pointer", fontSize: 14, fontWeight: 900, letterSpacing: ".08em", textTransform: "uppercase" } }, loading ? "Verificando..." : "Entrar \u2192")), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 16, display: "flex", gap: 10 } }, /* @__PURE__ */ React.createElement("div", { onClick: () => { if (window._dp) { window._dp.prompt(); window._dp = null; } else { const ibar = document.getElementById("ibar"); if (ibar) ibar.style.display = "flex"; } }, style: { flex: 1, display: "flex", alignItems: "center", gap: 8, background: "#0a1a0f", border: "1px solid #00e5a033", borderRadius: 10, padding: "10px 12px", cursor: "pointer" } }, /* @__PURE__ */ React.createElement(InstallIcon, null), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: "#00e5a0", fontFamily: "monospace" } }, "Instalar"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#3a6a4a", fontFamily: "monospace" } }, "Tela inicial"))), /* @__PURE__ */ React.createElement("div", { onClick: () => setCalcOpen(true), style: { flex: 1, display: "flex", alignItems: "center", gap: 8, background: "#0d1a26", border: "1px solid #1e3a4a", borderRadius: 10, padding: "10px 12px", cursor: "pointer" } }, /* @__PURE__ */ React.createElement(CalculatorIcon, null), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, fontWeight: 700, color: "#5aa8e0", fontFamily: "monospace" } }, "Calculadora"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: "#3a5a6a", fontFamily: "monospace" } }, "Produ\xE7\xE3o")))), calcOpen && /* @__PURE__ */ React.createElement(ProductionCalcModal, { onClose: () => setCalcOpen(false) }), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 14, textAlign: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#3a5a7a", fontFamily: "monospace" } }, "Desenvolvido por ", /* @__PURE__ */ React.createElement("span", { style: { color: "#4a8aff" } }, APP_DEV)), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#2a4a6a", marginTop: 3, fontFamily: "monospace" } }, APP_VERSION)));
}
...
function ComparisonModal({ data, onClose, onSaveName, selDay }) {
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, gap: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#3a5a7a", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 4 } }, "\u{1F3ED} ", line), onSaveName ? /* @__PURE__ */ React.createElement(Field, { label: "Produto", value: displayName, onSave: saveName, fullWidth: true }) : /* @__PURE__ */ React.createElement("div", { style: { fontSize: 15, fontWeight: 700, color: "#e0e8ff", lineHeight: 1.3 } }, displayName), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 11, color: "#3a5a7a", marginTop: 4 } }, selDay)), /* @__PURE__ */ React.createElement("button", { onClick: onClose, style: { background: "transparent", border: "none", color: "#5a6a8a", cursor: "pointer", fontSize: 20, flexShrink: 0 } }, "\u2715")),
...
function ProductionDashboard({ session, machines, onGoPanel, onLogout, onSettings, hideHeader, onUpdateMachine }) {
  ...
  function saveProductName(data, newName) {
    if (!onUpdateMachine || !newName || !newName.trim()) return;
    const m = (machines || []).find((x) => x.name === data.line);
    if (!m) return;
    const newHistory = (m.history || []).map((h) => {
      if (h === (data.rA && data.rA._rec) || h === (data.rB && data.rB._rec)) {
        return __spreadProps(__spreadValues({}, h), { produto: newName.trim() });
      }
      return h;
    });
...
function AutocompleteField({ label, value, onSave, storageKey, large, fullWidth, highlight }) {
  ...
  function saveSuggestion(val) {
    ...
    try {
      ...
      if (storageKey === "canaleta_operators") salvarOperatorsFirebase(updated);
      if (storageKey === "canaleta_products") salvarProductsFirebase(updated);
    } catch (e) {
    }
  }
  function start() {
    ...
    setOpen(true);
    if (storageKey === "canaleta_operators") {
      carregarOperatorsFirebase().then(function(d){ setSuggestions(d); });
    }
    if (storageKey === "canaleta_products") {
      carregarProductsFirebase().then(function(d){ setSuggestions(d); });
    }
  }
  function save(v) {
...
function HistoryScreen({ machines, singleMachine, summaryMode, onBack, onDeleteRecord, onUpdateRecord }) {
  ...
  const EDITABLE_KEYS = [["Operador", "operador"], ["Produto", "produto"], ["Ordem", "orderId"], ["In\xEDcio", "inicio"], ["T\xE9rmino", "termino"], ["Ciclo (s)", "ciclo"], ["Produ\xE7\xE3o real", "producaoReal"], ["Diferen\xE7a", "diferenca"], ["Peso total (kg)", "pesoTotal"], ["Perda material (kg)", "perdaKg"], ["RPM Extrusora", "rpmExtrusora"], ["RPM Coextrusora", "rpmCoextrusora"], ["Amperagem (A)", "amperagem"], ["RPM Puxador", "rpmPuxador"]];
