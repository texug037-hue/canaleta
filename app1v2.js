...
function ComparisonModal({ data, onClose, onSaveName, selDay }) {
  ...
  function editName() {
    const v = window.prompt("Editar nome da produ\xE7\xE3o:", displayName);
    if (v === null) return;
    const trimmed = v.trim();
    if (!trimmed) return;
    setDisplayName(trimmed);
    onSaveName && onSaveName(data, trimmed);
  }
